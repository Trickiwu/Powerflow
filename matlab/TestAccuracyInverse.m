...
for i = 1:num_train
    Y2 = data.V(i, [pv; ref])';
    a1 = [P(i, [pq; pv; ref])'; data.Q(i, pq)'];%%x1
    a2 = X22\(Y2 - X21 * a1 - C2);%%x2
    
    num_pq = size(pq,1);%%1
    num_pv = size(pv,1);%%3
    Q_pv = a2(1:num_pv);%%1:3
    Q_ref = a2(num_pv + 1:num_pv + 1);%%4:4
    ...
    Va = zeros(num_load, 1);
    Q = data.Q(i, :);
    V([pv; ref]) = data.V(i, [pv; ref]);  %%V of pv ref bus
    V(pq) = Y1(num_load + 1: num_load + num_pq);%%Y1=[Ang(L),Ang(S),P(r),V(L)]T  6:6
